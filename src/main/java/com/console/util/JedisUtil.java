...
public class JedisUtil {
	...
	/** 缓存生存时间 */
	private final int expire = 600000;
	/** 操作Key的方法 */
	public Keys KEYS;
	/** 对存储结构为String类型的操作 */
	...
	static {
		...
		// redis如果设置了密码：
		jedisPool = new JedisPool(config, "10.0.1.40", 6379, 10000, "Yunkong1234!");
		// redis未设置了密码：
		// jedisPool = new JedisPool(config, "47.104.91.79",9094);
	...
	public static void returnJedis(Jedis jedis , Boolean broken){
		if (null != jedis && null != jedisPool) {
			if (broken) {  
				jedisPool.returnBrokenResource(jedis);  
	        } else {  
	...
	public static void main(String[] args) {
		...
		try {
			...
			JedisUtil.getInstance().returnJedis(jedis , broken);
		}
